...
def calc_porosity_factor(fillFactor):
    """Calculate the porosity factor K for a given fill factor.

    Args:
        fillFactor (float): The soil's fill factor.

    Returns:
        float: The calculated porosity factor.
    """
...
def hapke(w, x, K):
    """Calculate Hapke's 1% error H function using the IMSA Model.
    ...
    Returns:
        float: The calculated H function.
    """
...
def G1(t_s, t_0, sensor_azimuth, solar_azimuth):
    """Calculate g, based on solar zenith, view zenith, and relative azimuth.
    ...
    Returns:
        float: The calculated g.
    """
    phi = sensor_azimuth - solar_azimuth

    g = np.arccos(np.cos(np.deg2rad(t_s))*np.cos(np.deg2rad(t_0)) + np.sin(np.deg2rad(t_s))*np.sin(np.deg2rad(t_0))*np.cos(np.deg2rad(phi)))
...
def G2(t_s, t_0, sensor_azimuth, solar_azimuth):
    ...
    g_p = np.arccos(np.cos(np.deg2rad(t_s))*np.cos(np.deg2rad(t_0)) - np.sin(np.deg2rad(t_s))*np.sin(np.deg2rad(t_0))*np.cos(np.deg2rad(phi)))
...
def phase_function(b1, b2, b3, b4, t_s, t_0, sensor_azimuth, solar_azimuth):
    """Calculate the phase function based on solar zenith, view zenith, relative azimuth, and coefficients.
    ...
    Returns:
        float: The calculated phase function.
    """
...
def k( b1, b2, b3, b4, t_s, t_0, B0, sensor_azimuth, solar_azimuth,  fillFactor, C):
    """
    ...
    p = phase_function(b1, b2, b3, b4, t_s, t_0, sensor_azimuth, solar_azimuth)
    K = (p * (1 + b_g))
    return K


def calc_refl(B0, t_s, t_0, sensor_azimuth, solar_azimuth, w0, b1, b2, b3, b4, fillFactor, C):
    """
    ...
    """
    x_s = np.cos(np.deg2rad(t_s))
    x_0 = np.cos(np.deg2rad(t_0))

    K = calc_porosity_factor(fillFactor)
    m = k(b1, b2, b3, b4, t_s, t_0, B0, sensor_azimuth, solar_azimuth, fillFactor, C)
    H_s = hapke(w0, x_s, K)
    H_0 = hapke(w0, x_0, K)
