...
def get_logistic_function_params(mwt_output,smc_input):
    ...
    wvl,_ = get_swap_hapke_data(path + files[0])
    ...
    for i in range(0, file_count):
        wavelength, L[i,:], = get_swap_hapke_data(path + files[num_argsort[i]])
...
def saveplots(wvl,mean_water_thickness,smc,r2_final,nrmse_final,wvl_final,K_final,psi_final,alpha_final,test_dat,img_output,wvl_m):
    ...
    # Calculate various statistics for r2_final and nrmse_final
    ...
    ind_mean = int(np.argwhere((wvl_final == find_nearest(wvl_final,wvl_m)) & (r2_final == find_nearest(r2_final[np.argwhere((wvl_final == find_nearest(wvl_final,wvl_m))).flatten()],mu)))[0].flatten())
    ...
    # Plotting various plots and saving them to img_output folder
    ...
    # Print and save model statistics
    t = PrettyTable(['---+---',' ---+---'])
    ...
    print(' ')
    print('##################################################################')
    print('|--- Original (SWAP)-Hapke retrieval output stats ----|')
    print(t)


    f = open('./outputs' + '/model_stat.csv', 'w')
    f.write('Original (SWAP)-Hapke retrieval output stats' )
    f.write('\r\n')
    f.write('\r\n')
...
solar_zenith_dry = 18.450
solar_azimuth_dry = 145.540
sensor_zenith_dry = 14.68386207226664
sensor_azimuth_dry = 288.0859496548972

w, B, b1, c1, b2, c2, h = perform_inversion_dry(
    reflectance_dry,
    solar_zenith_dry,
    sensor_zenith_dry,
    sensor_azimuth_dry,
...
def output_mw(csvfiles_wet):
    ...
    try:
        ...
        writer.writerow(('Wavelength', 'Single_scattering_albedo', 'Intensity_size_of_h',  'Coefficient_b1', 'Coefficient_c1', 'Coefficient_b2', 'Coefficient_c2', 'Water_level', 'h' ))
        for i in range(len(wavelength)):
            writer.writerow((wavelength[i],w[i], B[i], b1[i], c1[i], b2[i], c2[i], L[i],  h[i]))
    finally:
        flname.close()
    return 0
...
print('Finishing original (SWAP)-Hapke  retrieval, total time taken = ', (time.time() - start_time1) / 3600, 'hours')
