...
def get_curve_fit_data(inv_folder,
                       ...
                       curve_r2):
    """
    Given the input files, this function extracts the curve fit data.

    Args:
        inv_folder (str): The directory where inversion files are located.
        smc_folder (str): The directory where soil moisture content files are located.
        curve_K (str): The file path to curve fit parameter K.
        curve_psi (str): The file path to curve fit parameter psi.
        curve_alpha (str): The file path to curve fit parameter alpha.
        curve_r2 (str): The file path to curve fit parameter r2.

    Returns:
        tuple: Tuple containing mean water thickness, measured and estimated soil moisture content (SMC), normalized root mean square error (nrmse), r2 values, curve fit parameters K, psi, alpha, and sorted nrmse indices.
    """
    ...
    K = np.stack([K for _ in range(len(smc_meas))], axis=2)
    psi = get_L_eps_data(curve_psi).astype(float)
    psi = np.stack([psi for _ in range(len(smc_meas))], axis=2)
    alpha = get_L_eps_data(curve_alpha).astype(float)
    alpha = np.stack([alpha for _ in range(len(smc_meas))], axis=2)
...
def plot_est_meas_smc(ind_val,rms_sort_ind,smc_est,smc_meas,wavelength,sensor_zenith_dry,sensor_azimuth_dry,img_output):
    ...
    Args:
        ind_val (int): Index value for selecting best rms indices.
        rms_sort_ind (np.array): Array of sorted rms indices.
    ...
    plt.xlabel('Measured SMC', fontsize=20)
    plt.ylabel('Estimated SMC', fontsize=20)
...
def plot_logistic_function(ind_val,rms_sort_ind,smc_est,smc_meas,mean_water_thickness,K,psi,alpha,img_output):
    ...
    Args:
        ind_val (int): Index value in the sorted RMS array.
        rms_sort_ind (numpy array): Indices of the sorted RMS array.
...
def save_output_data(ind_val,rms_sort_ind,smc_est,smc_meas,wavelength,sensor_zenith_dry,sensor_azimuth_dry,K,psi,alpha,r2_val,smc_output):
    ...
    """
    Saves output data from Marmit retrieval, including estimated and measured soil moisture content (SMC),
