...
def get_logistic_function_params(mwt_output,smc_input):
    ...
    wvl,_,_ = get_modified_swap_hapke_data(path + files[0])
    ...
    for i in range(0, file_count):
        wavelength, L[i,:], epsilon[i,:], = get_modified_swap_hapke_data(path + files[num_argsort[i]])
...
def saveplots(wvl, mean_water_thickness, smc, r2_final, nrmse_final, wvl_final, K_final, psi_final, alpha_final,
    ...
    print(' ')
    print('##################################################################')
    print('|--- Modified (SWAP)-Hapke retrieval output stats ----|')
    print(t)


    f = open('./outputs' + '/model_stat.csv', 'w')
    f.write('Modified (SWAP)-Hapke retrieval output stats' )
    f.write('\r\n')
    f.write('\r\n')
...
w, B, b1, c1, b2, c2, fill, C = perform_inversion_dry(
    reflectance_dry,
    solar_zenith_dry,
    sensor_zenith_dry,
    sensor_azimuth_dry,
...
def output_mw(csvfiles_wet):
    ...
    L, epsilon = perform_inversion_wet(
        reflectance_wet,
        solar_zenith_wet,
        sensor_zenith_wet,
        sensor_azimuth_wet,
    ...
    try:
        ...
        writer.writerow(('Wavelength', 'Single_scattering_albedo', 'Intensity_size_of_h', 'Coefficient_b1', 'Coefficient_c1', 'Coefficient_b2', 'Coefficient_c2', 'Water_level', 'Efficiency', 'Fill', 'C' ))
        for i in range(len(wavelength)):
            writer.writerow((wavelength[i],w[i], B[i], b1[i], c1[i], b2[i], c2[i], L[i], epsilon[i], fill[i], C[i]))
    finally:
        flname.close()
    return 0
...
print('Finishing modified (SWAP)-Hapke retrieval, total time taken = ', (time.time() - start_time1) / 3600, 'hours')
