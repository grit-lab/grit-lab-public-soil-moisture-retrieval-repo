...
def get_curve_fit_data(inv_folder,
                       ...
                       curve_r2):
    """
    Processes the curve fit data and calculates the Normalized Root Mean Square (nrmse) of the estimated soil moisture content (SMC).

    This function loads various parameters such as single scattering albedo, hot spot effect, intensity size of h,
    coefficients b1, b2, c1, c2, and water level from csv files in the given inv_folder. It sorts and reshapes these
    parameters, and uses them along with the provided curve parameters to calculate the estimated SMC using the logistic function.

    Args:
        inv_folder (str): The path to the folder containing the input csv files.
        smc_folder (str): The path to the folder containing the soil moisture content data.
        curve_K (str): The path to the csv file containing the curve K parameter.
        curve_psi (str): The path to the csv file containing the curve psi parameter.
        curve_alpha (str): The path to the csv file containing the curve alpha parameter.
        curve_r2 (str): The path to the csv file containing the curve r2 parameter.

    Returns:
        mean_water_thickness (numpy.ndarray): An array containing the mean water thickness data.
        smc_meas (numpy.ndarray): An array containing the measured soil moisture content data.
        smc_est (numpy.ndarray): An array containing the estimated soil moisture content data.
        rms (numpy.ndarray): An array containing the normalized root mean square error of the estimated soil moisture content.
        r2_val (numpy.ndarray): An array containing the r2 values from the curve fit.
        K (numpy.ndarray): An array containing the K parameter values.
        psi (numpy.ndarray): An array containing the psi parameter values.
        alpha (numpy.ndarray): An array containing the alpha parameter values.
        rms_sort_ind (numpy.ndarray): An array containing the indices that would sort the nrmse array.
    """
    ...
    #smc_meas_3d = np.kron(np.ones((39,2151,1)),smc_meas)
