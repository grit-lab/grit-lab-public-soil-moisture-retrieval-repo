...
if dataset == 'alg':
    ...
    plt_smc_input = '../input/smc_data_refl/algodones_sample2.csv'
    ...
    plt_smc_input = '../input/smc_data_refl/nevada_sample2.csv'
    ...
    plt_smc_input = '../input/smc_data_refl/hogp_sample2.csv'
    ...
    plt_smc_input = '../input/smc_data_refl/hogb_sample2.csv'
...
def output_mw(csvfiles_wet):
    ...
    c1 = np.zeros(reflectance_wet.shape)
    b2 = np.zeros(reflectance_wet.shape)
    c2 = np.zeros(reflectance_wet.shape)
    L = np.zeros(reflectance_wet.shape)
    h = np.zeros(reflectance_wet.shape)

    for i in range(0, reflectance_wet.shape[0]):

        w[i, :], B[i, :], b1[i, :], c1[i, :], b2[i, :], c2[i, :], h[i, :] = perform_inversion_dry(
    ...
    c1_output = './' + results_folder + '/' + csvfiles_wet.rsplit('/')[-1].rsplit('_singlemode')[0] + '_coefficient_c1.csv'
    b2_output = './' + results_folder + '/' + csvfiles_wet.rsplit('/')[-1].rsplit('_singlemode')[0] + '_coefficient_b2.csv'
    c2_output = './' + results_folder + '/' + csvfiles_wet.rsplit('/')[-1].rsplit('_singlemode')[0] + '_coefficient_c2.csv'
    L_output = './' + results_folder + '/' + csvfiles_wet.rsplit('/')[-1].rsplit('_singlemode')[0] + '_water_level.csv'
    h_output = './' + results_folder + '/' + csvfiles_wet.rsplit('/')[-1].rsplit('_singlemode')[0] + '_hot_spot_effect.csv'
    ...
    np.savetxt(c1_output, c1, delimiter=',')
    np.savetxt(c2_output, c2, delimiter=',')
    np.savetxt(L_output, L, delimiter=',')
    np.savetxt(h_output, h, delimiter=',')
...
print('Finishing step 1: Hapke inversion to retrieve mean water thickness, time taken = ',
...
print('Finishing step 2: Hapke curve fitting to logistic function, time taken = ', (time.time() - start_time2) / 3600,
      'hours')
